...
import scrapy
import json
from Scrapy_SCGY_xmys.items import ScrapyScgyXmysItem
...
class GraspGdstcSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for i in range(1, 2):
            url = f'http://sjfb.gdstc.gd.gov.cn/sjfb/api/project/listTPlanCheck?size=10&current={i}&projectName=%E7%9F%B3%E6%9D%90'
            req = scrapy.Request(
                url,
                callback=self.parse,
                dont_filter=True,

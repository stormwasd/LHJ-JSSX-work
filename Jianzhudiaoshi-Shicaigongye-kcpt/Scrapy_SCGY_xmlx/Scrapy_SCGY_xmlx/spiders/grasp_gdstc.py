import json
import scrapy
from Scrapy_SCGY_xmlx.items import ScrapyScgyXmlxItem
...
class GraspGdstcSpider(scrapy.Spider):
    ...
    def start_requests(self, *args, **kwargs):
        for i in range(1, 2):
            url = f'http://sjfb.gdstc.gd.gov.cn/sjfb/api/project/listProjectInitial?size=10&current={i}&projectName=%E7%9F%B3%E6%9D%90'
            req = scrapy.Request(
                url,
                callback=self.parse,
                dont_filter=True,
    ...
    def parse(self, response):
        ...
        for i in jso:
            ...
            country = '中国'
            indic_name = '石材工业产业'
            issue_time = i.get('beginYear') + '-12-29'

            item = ScrapyScgyXmlxItem()

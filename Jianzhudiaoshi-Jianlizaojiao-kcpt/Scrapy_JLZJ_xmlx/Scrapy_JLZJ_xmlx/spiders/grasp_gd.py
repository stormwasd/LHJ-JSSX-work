import json
import scrapy
from Scrapy_JLZJ_xmlx.items import ScrapyJlzjXmlxItem
...
class GraspGdstcGdSpider(scrapy.Spider):
    ...
    def start_requests(self, *args, **kwargs):
        for i in range(1, 2):
            url = f'http://sjfb.gdstc.gd.gov.cn/sjfb/api/project/listProjectInitial?size=10&current={i}&projectName=%E7%9B%91%E7%90%86'
            req = scrapy.Request(
                url,
                callback=self.parse,
                dont_filter=True,
...
if __name__ == '__main__':
    ...
    cmd.execute(['scrapy', 'crawl', 'grasp_gd'])

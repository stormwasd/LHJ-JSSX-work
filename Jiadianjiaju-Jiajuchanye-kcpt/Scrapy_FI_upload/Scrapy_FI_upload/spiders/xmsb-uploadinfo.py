...
class UploadInfo(scrapy.Spider):
    ...
    def __init__(*args,**kwargs):
        ...
        collection = db['lhj_kcpt_xmsb']  # 获取collection
        url_info = 'http://192.168.0.11:10020/dataApp/sciencePlatform/addList/apply'
        count = 0
        ...
        for item in collection.find(filter={'cleaning_status': 0}, no_cursor_timeout=True):
            ...
            # 修改成验收的字段类型
            form_data = [{
                "amountDeclared": '' if 'amount_declared' not in dict(item).keys() else item['amount_declared'],
...
if __name__ == '__main__':
    ...
    cmdline.execute('scrapy crawl xmsb-uploadinfo'.split())

import scrapy
from Scrapy_CCY_kjzc.items import ScrapyCcyKjzcItem
...
class GraspZgchawangNewsSpider(scrapy.Spider):
    name = 'grasp_zgchawang_news'
    ...
    def start_requests(self):
        for i in range(1, 2):
            url = f"http://www.zgchawang.com/news/list/36/{i}/"
            req = scrapy.Request(
                url,
                callback=self.parse,
                dont_filter=True,
    ...
    def parse(self, response):
        ...
        # max_page = response.xpath("//div[@class='pages']/label/span/text()").extract_first()
        for i in range(len(url_list)):
            ...
            yield req
        # next_page = response.xpath("//div[@class='pages']/a[last()]/@href").extract_frist()
        # if next_page and max_page not in next_page:
        #     rep = scrapy.Request(
        #         next_page,
        #         callback=self.parse,
        #         dont_filter=True,
...
if __name__ == '__main__':
    ...
    cmd.execute(['scrapy', 'crawl', 'grasp_zgchawang_news'])

...
import scrapy
import json
from Scrapy_CCY_xmys.items import ScrapyCcyXmysItem
...
class GraspGdstcSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for i in range(1, 2):
            url = f'http://sjfb.gdstc.gd.gov.cn/sjfb/api/project/listTPlanCheck?size=10&current={i}&projectName=%E8%8C%B6'
            req = scrapy.Request(
                url,
                callback=self.parse,
                dont_filter=True,

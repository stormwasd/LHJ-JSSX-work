...
import scrapy
import json
from Scrapy_CCY_xmsb.items import ScrapyCcyXmsbItem
...
class GraspGdstcSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for i in range(1, 2):
            url = f'http://sjfb.gdstc.gd.gov.cn/sjfb/api/project/listPlanDeclare?size=10&current={i}&name=%E8%8C%B6'
            req = scrapy.Request(
                url,
                callback=self.parse,
                dont_filter=True,

import scrapy
from Scrapy_CCY_cxlm.items import ScrapyCcyCxlmItem
...
class GraspZgchawangLmSpider(scrapy.Spider):
    name = 'grasp_zgchawang_lm'
    ...
    def start_requests(self):
        for i in range(1, 2):
            url = f"http://www.zgchawang.com/news/search.php?moduleid=21&spread=0&kw=%E8%81%94%E7%9B%9F&page={i}"
            req = scrapy.Request(
                url,
                callback=self.parse,
                dont_filter=True,
...
if __name__ == '__main__':
    ...
    cmd.execute(['scrapy', 'crawl', 'grasp_zgchawang_lm'])

...
import scrapy
import json
from Scrapy_DS_xmys.items import ScrapyDsXmysItem


class GraspGdstcSpider(scrapy.Spider):
    name = 'grasp_sjfb_gdstc'
    ...
    def start_requests(self):
        for i in range(1, 2):
            url = f'http://sjfb.gdstc.gd.gov.cn/sjfb/api/project/listTPlanCheck?size=10&current={i}&projectName=%E9%9B%95%E5%A1%91'
            req = scrapy.Request(
                url,
                callback=self.parse,
                dont_filter=True,
...
if __name__ == '__main__':
    ...
    cmd.execute(['scrapy', 'crawl', 'grasp_sjfb_gdstc'])

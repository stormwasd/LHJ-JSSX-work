...
import scrapy
import json
from Scrapy_YCCY_xmys.items import ScrapyYccyXmysItem


class GraspGdstcSpider(scrapy.Spider):
    name = 'grasp_gdstc_gd_gov'
    ...
    def start_requests(self):
        for i in range(1, 2):
            url = f'http://sjfb.gdstc.gd.gov.cn/sjfb/api/project/listTPlanCheck?size=10&current={i}&projectName=%E7%83%9F%E8%8D%89'
            req = scrapy.Request(
                url,
                callback=self.parse,
                dont_filter=True,
...
if __name__ == '__main__':
    ...
    cmd.execute(['scrapy', 'crawl', 'grasp_gdstc_gd_gov'])